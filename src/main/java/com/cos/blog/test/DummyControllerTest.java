...
//htmp파일이 아니라 data를 리턴해주는 controller = RestController
@RestController
public class DummyControllerTest {
	...
	public User updateUser(@PathVariable int id, @RequestBody User requestUser) { //json 데이터를 요청 => Java Object(MessageConverter의 Jackson라이브러리가 변환해서 받아줌)
		...
		//requestUser.setUsername("taegon");
	...
	//http://localhost:8080/blog/dummy/user/page
	...
	public User detail(@PathVariable int id) {
		// user/4를 찾으면 내가 데이터베이스에서 못찾아오게되면 user가 null이됨
		// 그럼 return할때 null이 리턴되니 문제가 발생
		// Optional로 User객체를 감싸서 가져올테니 null인지 아닌지 판단하여 return해라
